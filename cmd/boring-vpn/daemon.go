//
// Logic for launching and controlling the boring-vpn daemon.
//
...
var doNotSpawn = os.Getenv("BORING_VPN_NO_SPAWN") != ""
...
func connectDaemon() (net.Conn, error) {
	return net.Dial("unix", vpn.Socket)
}

// ensureDaemon starts the daemon if not already running, provided
// that the BORING_VPN_NO_SPAWN environment variable is not set. It will
		...
		case <-wait.C:
			...
			if errors.As(err, &ce) {
				...
				if err := killDaemon(ctx); err != nil {
					info := "Please kill the old daemon process manually (e.g., `killall boring-vpn`)." +
			...
			if doNotSpawn {
				return fmt.Errorf("not running and BORING_VPN_NO_SPAWN is set")
			}
			if !launching {
				// Fail fast with an actionable message instead of repeatedly
				// spawning a daemon that will just exit immediately: the
				// vpn daemon refuses to start at all without privileges.
				if !vpnd.HasPrivileges() {
					return fmt.Errorf("%s", vpnd.PrivilegeError())
				}
...
func sendCmd(cmd vpn.Cmd) (*vpn.Resp, error) {
...
func probeDaemon() error {
	resp, err := sendCmd(vpn.Cmd{Kind: vpn.Nop})
	if err != nil {
		return err
	}
...
func killDaemon(ctx context.Context) error {
	resp, err := sendCmd(vpn.Cmd{Kind: vpn.Shutdown})
	if err != nil {
	...
	for {
		...
		case <-time.After(wait):
			if ln, err := net.Listen("unix", vpn.Socket); err == nil {
...
func launchDaemon() error {
	...
	pid, err := launchDaemonOS(ex, vpn.Flag)
	if err != nil {
		return err
	}
